...
import { Heart, MapPin, Ruler, Users, ChevronRight, Calendar, Sparkles } from 'lucide-react';
...
export const HouseCard: React.FC<HouseCardProps> = ({ house, isFavorite, onToggleFavorite, onSelect }) => {
  return (
    <div 
      ...
      <div className="relative h-44 md:h-52 overflow-hidden flex-shrink-0">
        ...
        <button 
          ...
          className={`absolute top-4 right-4 p-2.5 rounded-xl transition-all active:scale-90 ${
            isFavorite ? 'bg-red-500 text-white shadow-xl scale-110' : 'bg-white/80 dark:bg-slate-800/80 text-slate-600 dark:text-slate-400 hover:bg-white dark:hover:bg-slate-700'
          }`}
        >
      ...
      <div className="p-6 flex flex-col flex-1">
        ...
        <div className="flex items-center justify-between mt-auto pt-4 border-t border-slate-50 dark:border-slate-800">
          <div className="flex flex-col">
            <span className="text-xl font-black text-blue-600 dark:text-blue-400 leading-none mb-1">Ksh {house.price.toLocaleString()}</span>
            <span className="text-[9px] text-slate-400 dark:text-slate-500 font-black uppercase tracking-widest">Month</span>
          </div>
